...
let selectedCountryForQuiz = null;
let generalQuizData = null;
let generalQuizAnswers = [];
let challengeTimer = null;
let challengeNamedCountries = new Set();
let challengeCountriesList = [];
let currentTab = 'home';
...
async function initializeApp() {
  try {
    const response = await fetch(`${API_BASE}/all?fields=name,capital,population,region,subregion,flags`);
...
function setupEventListeners() {
  ...
  document.getElementById('back-home-from-results-btn').addEventListener('click', goToHome);
  document.getElementById('back-home-from-general-results-btn').addEventListener('click', goToHome);
  document.getElementById('back-home-from-challenge-btn').addEventListener('click', goToHome);
  document.getElementById('retake-quiz-btn').addEventListener('click', retakeCountryQuiz);
  document.getElementById('retake-general-quiz-btn').addEventListener('click', retakeGeneralQuiz);
...
function generateCountryQuiz(country) {
  const questions = [];
  ...
  questions.push({
    ...
    options: generateCapitalOptions(country)
  });
  
  // Population question
  questions.push({
    type: 'population',
    text: `Approximately how many people live in ${country.name.common}?`,
    correct: getPopulationRange(country.population),
  ...
  questions.push({
    type: 'region',
    text: `Which region is ${country.name.common} located in?`,
  ...
  questions.push({
    type: 'subregion',
    text: `Which subregion is ${country.name.common} part of?`,
    correct: country.subregion || 'N/A',
    options: generateSubregionOptions(country.region)
  });
  ...
  questions.push({
    ...
    options: generateFlagOptions(country)
  });
  
  return questions;
}

function generateCapitalOptions(correctCountry) {
  const options = [correctCountry.capital ? correctCountry.capital[0] : 'N/A'];
  while (options.length < 4) {
    const randomCountry = allCountries[Math.floor(Math.random() * allCountries.length)];
    const capital = randomCountry.capital ? randomCountry.capital[0] : 'N/A';
...
function generatePopulationOptions(correctPopulation) {
  const ranges = [
    'Less than 1 million',
    '1-10 million',
    '10-50 million',
...
function getPopulationRange(population) {
  if (population < 1000000) return 'Less than 1 million';
  if (population < 10000000) return '1-10 million';
  if (population < 50000000) return '10-50 million';
  if (population < 100000000) return '50-100 million';
  if (population < 500000000) return '100-500 million';
  return 'More than 500 million';
}

function generateSubregionOptions(region) {
  const subregions = {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (questionType === 0) {
      ...
      questions.push({
        ...
        correct: randomCountry.capital ? randomCountry.capital[0] : 'N/A',
        options: generateCapitalOptions(randomCountry)
      });
    } else if (questionType === 1) {
      // Population
      questions.push({
        type: 'population',
        text: `Approximately how many people live in ${randomCountry.name.common}?`,
        correct: getPopulationRange(randomCountry.population),
        options: generatePopulationOptions(randomCountry.population)
      });
    } else if (questionType === 2) {
      // Region
      questions.push({
        type: 'region',
        text: `Which region is ${randomCountry.name.common} located in?`,
        correct: randomCountry.region,
        options: ['Africa', 'Americas', 'Asia', 'Europe', 'Oceania']
      });
    } else {
      // Flag
      questions.push({
        type: 'flag',
        text: `Which country's flag is this?`,
        correct: randomCountry.name.common,
        flagUrl: randomCountry.flags.png,
        options: generateFlagOptions(randomCountry)
      });
    }
  }
